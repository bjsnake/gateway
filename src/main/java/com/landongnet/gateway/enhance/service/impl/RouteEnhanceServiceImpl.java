...
import com.github.snake.rock.common.utils.IpUtil;
import com.github.snake.rock.common.utils.JsonUtils;
import com.github.snake.rock.common.utils.ResponseUtils;
import com.landongnet.gateway.enhance.entity.*;
import com.landongnet.gateway.enhance.service.*;
...
public class RouteEnhanceServiceImpl implements RouteEnhanceService {
  ...
  public Mono<Void> filterBlackList(ServerWebExchange exchange) {
    ...
    try {
      ...
      if (originUri != null) {
        ...
        if (forbid.get()) {
          return ResponseUtils.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
            HttpStatus.NOT_ACCEPTABLE, R.error(500,"黑名单限制，禁止访问"));
        }
      } else {
  ...
  public Mono<Void> filterRateLimit(ServerWebExchange exchange) {
    ...
    try {
      ...
      if (originUri != null) {
        ...
        if (o != null) {
          RateLimitRule rule =  JsonUtils.jsonCovertToObject(o.toString(),RateLimitRule.class);
  ...
  public void saveRequestLogs(ServerWebExchange exchange) {
    ...
    if (!StringUtils.equalsIgnoreCase(TOKEN_CHECK_URL, originUri.getPath())) {
      ...
      ServerHttpRequest request = exchange.getRequest();
      String ipAddress = IpUtil.getServerHttpRequestIpAddress(request);
  ...
  private void doBlackListCheck(AtomicBoolean forbid, Set<Object> blackList, URI uri, String requestMethod) {
    for (Object o : blackList) {
      BlackList b = JsonUtils.jsonCovertToObject(o.toString(), BlackList.class);
  ...
  private Mono<Void> doRateLimitCheck(AtomicBoolean limit, RateLimitRule rule, URI uri,
    ...
    if (limit.get()) {
      ...
      if (count == 0) {
        ...
        return ResponseUtils.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
          HttpStatus.TOO_MANY_REQUESTS,R.builder().code(503).message("访问频率超限，请稍后再试").build());
      } else {
  ...
  private boolean between(LocalTime from, LocalTime to) {
